  ...
  return <div className="min-h-screen bg-background">
      ...
      top: `${stickyTop}px`
        ...
        <div className="container mx-auto px-4 py-4">
          ...
          <div className="flex items-center gap-2 mb-2">
            <div className="flex items-center flex-wrap gap-2 flex-1">
              ...
              return <button key={category.id} onClick={() => handleCategoryClick(category.id)} className={cn("flex-shrink-0 px-3 py-1.5 font-heading text-xs uppercase transition-all whitespace-nowrap border", isActive ? "bg-primary text-primary-foreground shadow-brutal-sm border-primary" : "bg-background text-foreground hover:bg-muted border-foreground/20")}>
                    ...
                    {count > 0 && <span className={cn("ml-1.5 text-[10px]", isActive ? "text-primary-foreground/80" : "text-muted-foreground")}>
                        ({count})
                      </span>}
                  </button>;
            })}
            ...
            <Collapsible open={isFilterOpen}>
              <CollapsibleContent>
                <div className="pb-4">
                  ...
                  {filteredSubcategories.length === 0 ? <p className="text-xs text-muted-foreground">No hay subcategorías para esta categoría</p> : <div className="flex flex-wrap gap-2">
          ...
          <div className="items-center mb-2 flex-wrap gap-2 flex-1 flex flex-row">
            {/* View mode toggle */}
          ...
          }} className={cn("h-8 px-2 flex-shrink-0 border", isSearchOpen && "bg-primary text-primary-foreground")}>
      ...
      {/* Equipment Modal */}
      <EquipmentModal equipment={selectedEquipment} open={modalOpen} onOpenChange={setModalOpen} onAddToCart={handleAddToCart} getCartQuantity={getCartQuantity} canAddMore={canAddMore} onViewDetails={item => {
      setSelectedEquipment(item);
    }} />
    </div>;
};
