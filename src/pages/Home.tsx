...
const Home = () => {
  ...
  return <div className="min-h-screen bg-background">
      ...
      <section className="relative h-screen overflow-hidden border-b-4 border-foreground">
        ...
        <div ref={scrollContainerRef} className="relative z-10 h-full overflow-x-auto overflow-y-hidden scroll-smooth hide-scrollbar" style={{
        scrollSnapType: "x mandatory"
      }}>
          <div className="flex h-full">
            {spaces360.map((space, index) => <div key={space.id} className="space-card flex-shrink-0 w-screen h-full relative overflow-hidden" style={{
            ...
            transition: "transform 0.1s ease-out"
                ...
                <div className="w-full h-full bg-gray-dark/50 flex items-center justify-center relative">
                  <div className="text-center z-10 relative p-4 sm:p-6 lg:p-8">
                    ...
                    <div className="mt-4 sm:mt-6 lg:mt-8 flex flex-wrap gap-2 sm:gap-4 justify-center">
                      {space.cta && <Button asChild variant="hero" size="sm" className="flex-1 sm:flex-none sm:text-base">
                ...
                <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-3 z-20">
                  {spaces360.map((_, i) => <div key={i} className={`h-2 rounded-full transition-all ${i === index ? "w-12 bg-primary" : "w-2 bg-background/40"}`} />)}
                </div>
              </div>)}
          </div>
        </div>
