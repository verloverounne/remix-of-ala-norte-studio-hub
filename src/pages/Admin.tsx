...
const Admin = () => {
  ...
  const [newSpacePeriod, setNewSpacePeriod] = useState({
    start_date: "",
    end_date: ""
  });
  ...
  const fetchEquipment = async () => {
    const {
      data
    } = await supabase.from("equipment").select(`*, categories!category_id (*), subcategories (*)`).order("name");
    if (data) {
  ...
  const fetchSpaces = async () => {
    const {
      data
    } = await supabase.from("spaces").select("*").order("order_index");
    if (data) {
      const transformed = data.map((item: any) => ({
        ...item,
        images: Array.isArray(item.images) ? item.images : [],
  ...
  const fetchContactInfo = async () => {
    const {
      data
    } = await supabase.from("contact_info").select("*").limit(1).single();
    if (data) {
      setContactInfo({
  ...
  const fetchUnavailabilityPeriods = async (equipmentId: string) => {
    const {
      data,
      error
    } = await supabase.from("equipment_unavailability").select("*").eq("equipment_id", equipmentId).order("start_date", {
      ascending: true
    });
    if (!error && data) {
  ...
  const handleAddUnavailabilityPeriod = async () => {
    ...
    const {
      error
    } = await supabase.from("equipment_unavailability").insert({
    ...
    if (error) {
      ...
      toast({
        title: "AGREGADO",
        description: "Periodo de no disponibilidad agregado"
      });
      setNewPeriod({
        start_date: "",
        end_date: ""
  ...
  const handleDeleteUnavailabilityPeriod = async (periodId: string) => {
    ...
    const {
      error
    } = await supabase.from("equipment_unavailability").delete().eq("id", periodId);
    if (error) {
      toast({
        title: "ERROR",
        description: error.message,
  ...
  const fetchSpaceUnavailabilityPeriods = async (spaceId: string) => {
    const {
      data,
      error
    } = await supabase.from("space_unavailability").select("*").eq("space_id", spaceId).order("start_date", {
      ascending: true
    });
    if (!error && data) {
  ...
  const handleAddSpaceUnavailabilityPeriod = async () => {
    ...
    const {
      error
    } = await supabase.from("space_unavailability").insert({
  ...
  const handleMassPriceUpdate = async () => {
    ...
    if (isNaN(percentage)) {
      toast({
        title: "ERROR",
        description: "Porcentaje inválido",
        variant: "destructive"
      });
      return;
    }
    const updates = equipment.map(item => {
      ...
      return supabase.from("equipment").update({
    ...
    if (hasError) {
      toast({
        title: "ERROR",
        description: "Error al actualizar algunos precios",
        variant: "destructive"
      });
    } else {
      toast({
        title: "ACTUALIZADO",
        description: `Precios actualizados ${percentage > 0 ? "+" : ""}${percentage}%`
      });
      setMassPercentage("");
      fetchEquipment();
    }
  ...
  const handleUpdateContactInfo = async () => {
    ...
    const {
      data: existing
    } = await supabase.from("contact_info").select("id").limit(1).single();
    let error;
    if (existing) {
      // Update existing record
      ({
        error
      } = await supabase.from("contact_info").update(contactInfo).eq("id", existing.id));
    } else {
      // Insert new record
      ({
        error
      } = await supabase.from("contact_info").insert(contactInfo));
    }
    if (error) {
      toast({
        title: "ERROR",
  ...
  const filteredSubcategories = newEquipment.category_id ? subcategories.filter(s => s.category_id === newEquipment.category_id) : [];
  ...
  const handleUpdateStockFromCSV = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const row = lines[i].match(/("([^"]|"")*"|[^;]*)(;("([^"]|"")*"|[^;]*))*/) ? lines[i].split(/;(?=(?:[^"]*"[^"]*")*[^"]*$)/).map(f => f.replace(/^"|"$/g, "").trim()) : lines[i].split(";").map(f => f.replace(/"/g, "").trim());
      ...
      toast({
        title: "PROCESANDO",
        description: `Analizando ${csvStockMap.size} productos del CSV...`
      });
      ...
      toast({
        title: "ERROR",
        description: error.message || "Error al actualizar stock",
        variant: "destructive"
      });
    } finally {
  ...
  const handleImportEquipment = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast({
        title: "PROCESANDO",
        description: "Eliminando equipos existentes..."
      });
      const {
        error: deleteError
      } = await supabase.from("equipment").delete().neq("id", "00000000-0000-0000-0000-000000000000"); // Delete all rows
      ...
      for (let i = 0; i < equipmentToInsert.length; i += batchSize) {
        ...
        const {
          error: insertError
        } = await supabase.from("equipment").insert(batch);
        if (insertError) {
                ...
                <CardContent className="space-y-4">
                  <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-end">
                    <div className="flex-1 space-y-2">
                      ...
                      <Input type="number" placeholder="10" step="0.1" value={massPercentage} onChange={e => setMassPercentage(e.target.value)} />
                    </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-2 max-h-96 overflow-y-auto">
                    {equipment.map(item => <div key={item.id} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 p-3 border rounded">
                        <p className="flex-1 font-heading text-sm sm:text-base truncate">{item.name}</p>
                        <div className="flex gap-2 flex-1 sm:flex-initial">
                          <Input type="number" defaultValue={item.price_per_day} onChange={e => setIndividualPrices({
                        ...individualPrices,
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input placeholder="+54 11 1234-5678" value={contactInfo.whatsapp} onChange={e => setContactInfo({
                    ...contactInfo,
                  ...
                  <div className="space-y-2">
                    <Label>Email</Label>
                    <Input type="email" placeholder="info@alanorte.com" value={contactInfo.email} onChange={e => setContactInfo({
                    ...contactInfo,
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea placeholder="Mensaje para cotizaciones..." rows={4} value={contactInfo.quote_message} onChange={e => setContactInfo({
                    ...contactInfo,
                    quote_message: e.target.value
                  })} />
                  </div>
 ...
 {/* eliminasr truido de la pestaña backup 
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="max-w-[95vw] sm:max-w-3xl max-h-[90vh] overflow-y-auto mx-2 sm:mx-auto">
          ...
          {editingEquipment && <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
              <div className="space-y-2">
                <Label>Nombre *</Label>
                <Input value={editingEquipment.name} onChange={e => setEditingEquipment({
              ...editingEquipment,
              name: e.target.value
            })} />
              </div>
              <div className="space-y-2">
                ...
                <Input value={editingEquipment.name_en || ""} onChange={e => setEditingEquipment({
              ...editingEquipment,
              ...
              <div className="space-y-2">
                ...
                <Select value={editingEquipment.category_id || ""} onValueChange={v => setEditingEquipment({
              ...editingEquipment,
              category_id: v,
              subcategory_id: null
            })}>
              ...
              <div className="space-y-2">
                ...
                <Select value={editingEquipment.subcategory_id || ""} onValueChange={v => setEditingEquipment({
              ...editingEquipment,
              subcategory_id: v
            })}>
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    {editFilteredSubcategories.map(sub => <SelectItem key={sub.id} value={sub.id}>
                        {sub.name}
                      </SelectItem>)}
                  </SelectContent>
                </Select>
              ...
              <div className="space-y-2">
                <Label>Marca</Label>
                <Input value={editingEquipment.brand || ""} onChange={e => setEditingEquipment({
              ...editingEquipment,
              ...
              <div className="space-y-2">
                <Label>Modelo</Label>
                <Input value={editingEquipment.model || ""} onChange={e => setEditingEquipment({
              ...editingEquipment,
              ...
              <div className="space-y-2">
                ...
                <Input type="number" value={editingEquipment.price_per_day} onChange={e => setEditingEquipment({
              ...editingEquipment,
              ...
              <div className="space-y-2 md:col-span-2">
                <EquipmentImageManager imageUrl={editingEquipment.image_url || null} images={editingEquipment.images || []} onImageUrlChange={url => setEditingEquipment({
              ...editingEquipment,
              image_url: url
            })} onImagesChange={images => setEditingEquipment({
              ...editingEquipment,
              images
            })} />
              </div>
              <div className="space-y-2 md:col-span-2">
                <div className="flex items-center justify-between">
                  ...
                  <Switch checked={editingEquipment.featured || false} onCheckedChange={checked => setEditingEquipment({
                ...editingEquipment,
                featured: checked
              })} />
                </div>
              ...
              <div className="md:col-span-2 border-t-2 border-foreground pt-6 mt-6">
                  ...
                  <CardContent className="pt-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-2">
                        ...
                        <Input type="date" value={newPeriod.start_date} onChange={e => setNewPeriod({
                      ...newPeriod,
                      ...
                      <div className="space-y-2">
                        ...
                        <Input type="date" value={newPeriod.end_date} onChange={e => setNewPeriod({
                      ...newPeriod,
                ...
                {/* List of periods */}
                <div className="space-y-2">
                  {unavailabilityPeriods.length === 0 ? <p className="text-sm text-muted-foreground text-center py-4 border-2 border-dashed rounded">
                      No hay periodos de no disponibilidad configurados
                    </p> : unavailabilityPeriods.map(period => <div key={period.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-3 border-2 rounded bg-muted/30">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          <CalendarIcon className="h-4 w-4 text-muted-foreground shrink-0" />
                          <div className="min-w-0">
                            <p className="font-heading text-xs sm:text-sm break-words">
                              {format(new Date(period.start_date), "d 'de' MMMM, yyyy", {
                        locale: es
                      })}
                              {" → "}
                              {format(new Date(period.end_date), "d 'de' MMMM, yyyy", {
                        locale: es
                      })}
                            </p>
                            <p className="text-xs text-muted-foreground">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => handleDeleteUnavailabilityPeriod(period.id)} className="text-destructive hover:text-destructive shrink-0 self-end sm:self-auto">
                          <X className="h-4 w-4" />
                        </Button>
                      </div>)}
                </div>
