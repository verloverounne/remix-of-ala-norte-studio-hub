  ...
  return <section className="relative h-screen overflow-hidden border-b-4 border-foreground">
      ...
      loop: true
        ...
        <CarouselContent className="h-full -ml-0">
          {slidesWithVideos.map((slide, index) => <CarouselItem key={slide.id} className="h-full pl-0">
              <div className="relative h-screen w-full overflow-hidden">
                {slide.video_url ? <video ref={el => videoRefs.current[index] = el} src={slide.video_url} className="w-full h-full object-cover transition-[filter] duration-300 ease-out" style={{
              filter: 'blur(5px)'
            }} loop muted={muted} playsInline onMouseEnter={e => {
                ...
                <div className="absolute inset-0 flex items-center justify-center z-10">
                  <div className="text-center p-4 sm:p-6 lg:p-8 max-w-4xl">
                    ...
                    {slide.cta && <Button asChild variant="hero" size="lg" className="text-base sm:text-lg">
                        <Link to={slide.cta.link} className="border-0">{slide.cta.label}</Link>
                      </Button>}
                  </div>
                </div>
              </div>
      ...
      {/* Mute/Unmute Button */}
      {videos.length > 0 && <button onClick={() => setMuted(!muted)} className="absolute top-24 right-4 z-20 p-3 bg-background/20 backdrop-blur-sm rounded-full hover:bg-background/40 transition-colors" aria-label={muted ? "Activar sonido" : "Silenciar"}>
