          ...
          <nav className="w-full flex items-center justify-center px-4 py-3 backdrop-blur-sm">
            <div className="flex h-10 lg:h-14 items-center justify-center gap-4">
              ...
              <div className="flex flex-wrap items-center gap-1">
                {navigation.map(item => <Link key={item.name} to={item.href} className={`px-4 xl:px-6 h-10 xl:h-12 flex items-center justify-center bg-background font-heading text-xs xl:text-sm tracking-wider border transition-none flex-shrink-0 ${location.pathname === item.href ? "bg-primary text-primary-foreground border-foreground shadow-brutal-sm" : "bg-transparent border-transparent hover:border-foreground hover:shadow-brutal-sm"}`}>
                    {item.name}
                  </Link>)}
              </div>
