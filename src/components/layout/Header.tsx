...
export const Header = () => {
  ...
  return <>
      ...
      <header className={`fixed top-0 left-0 right-0 z-[55] bg-background border-b border-foreground transition-transform duration-300 ${isMobile ? mobileMenuOpen ? "translate-y-0" : "-translate-y-full" : isVisible || isHovering ? "translate-y-0" : "-translate-y-full"}`} onMouseEnter={!isMobile ? handleMouseEnter : undefined} onMouseLeave={!isMobile ? handleMouseLeave : undefined}>
        <nav className="container mx-auto px-3 sm:px-6 lg:px-8">
          <div className="flex h-10 lg:h-14 items-center justify-between">
            ...
            <div className="flex flex-wrap items-center gap-1 sm:gap-2">
              ...
              <Button variant="ghost" size="icon" onClick={toggleTheme} className="hidden sm:inline-flex h-10 w-10 xl:h-12 xl:w-12" aria-label={theme === "light" ? "Cambiar a modo oscuro" : "Cambiar a modo claro"}>
          ...
          {mobileMenuOpen && <div ref={menuRef} className="lg:hidden pb-4 mt-2 relative z-[56]" onTouchStart={resetAutoCloseTimer} onMouseMove={resetAutoCloseTimer}>
              ...
              WebkitTapHighlightColor: 'rgba(0,0,0,0.1)'
                ...
                <div className="flex items-center justify-between px-4 py-3 bg-background">
                  ...
                  <button type="button" onClick={toggleTheme} className="h-10 w-10 flex items-center justify-center touch-manipulation bg-muted rounded" aria-label={theme === "light" ? "Cambiar a modo oscuro" : "Cambiar a modo claro"} style={{
                WebkitTapHighlightColor: 'transparent'
              }}>
