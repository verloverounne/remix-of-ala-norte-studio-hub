...
export const CartSidebar = ({
  ...
  const CartContent = () => <>
      ...
      <div className="overflow-y-auto flex-1 p-4">
        {items.length === 0 ? <div className="text-center py-8">
            ...
            {items.map(item => <div key={item.id} className="p-3 bg-card">
                ...
                <div className="flex items-center justify-between mt-3 pt-3">
                  <div className="flex items-center gap-2">
                    <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-7 h-7 flex items-center justify-center hover:bg-foreground hover:text-background transition-colors">
                      <Minus className="w-3 h-3" />
                    </button>
                    <span className="font-heading text-sm w-6 text-center">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.id, item.quantity + 1)} disabled={item.stockQuantity !== undefined && item.quantity >= item.stockQuantity} className="w-7 h-7 border-2 border-foreground flex items-center justify-center hover:bg-foreground hover:text-background transition-colors disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:text-foreground">
                      <Plus className="w-3 h-3" />
                    </button>
                    {item.stockQuantity !== undefined && <span className="text-xs text-muted-foreground ml-1">(máx: {item.stockQuantity})</span>}
                  </div>
      ...
      {/* Footer */}
      {items.length > 0 && <div className="p-4 bg-muted/30">
    ...
    return <>
        ...
        <div className="fixed bottom-4 center z-40">
          <button onClick={() => setDrawerOpen(true)} className="relative bg-primary text-primary-foreground p-4 shadow-brutal-sm hover:scale-105 transition-transform py-[12px] px-[12px] my-[8px]">
            ...
            {totalItems > 0 && <span className="absolute -top-2 -right-2 text-destructive-foreground text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center bg-[#a59a93]">
                {totalItems}
              </span>}
          </button>
        </div>
  ...
  return <div className="sticky z-20 bg-card shadow-brutal" style={{
    top: `${stickyTop}px`
  }}>
      <CartContent />
    </div>;
