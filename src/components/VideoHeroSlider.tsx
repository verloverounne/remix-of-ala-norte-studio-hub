...
export const VideoHeroSlider = ({
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      const {
        data
      } = await supabase.from("gallery_images").select("*").eq("page_type", pageType).order("order_index");
      if (data) {
  ...
  return <section className="relative h-screen overflow-hidden">
      ...
      loop: true
        ...
        <CarouselContent className="h-full -ml-0">
          {videos.map((video, index) => <CarouselItem key={video.id} className="h-full pl-0">
              <div className="relative h-screen w-full overflow-hidden">
                <video ref={el => videoRefs.current[index] = el} src={video.video_url} className="w-full h-full object-cover" autoPlay={index === 0} loop muted={muted} playsInline />
      ...
      {/* Mute/Unmute Button */}
      <button onClick={() => setMuted(!muted)} className="absolute top-24 right-4 z-20 p-3 bg-background/20 backdrop-blur-sm rounded-full hover:bg-background/40 transition-colors" aria-label={muted ? "Activar sonido" : "Silenciar"}>
      ...
      {/* Navigation dots */}
      {videos.length > 1 && <div className="absolute bottom-32 left-1/2 -translate-x-1/2 flex gap-3 z-20">
          {videos.map((_, index) => <button key={index} onClick={() => api?.scrollTo(index)} className={`h-2 rounded-full transition-all ${index === currentSlide ? "w-12 bg-primary" : "w-2 bg-background/40"}`} aria-label={`Ir a video ${index + 1}`} />)}
        </div>}
    </section>;
};
const HeroContent = ({
  ...
  space: Space;
    ...
    <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-heading font-bold mb-4 text-gray-50">
    ...
    {space.discount_text && <div className="inline-flex items-center gap-2 bg-primary/20 border-2 border-primary px-4 py-2 rounded-lg mb-6 border-none">
