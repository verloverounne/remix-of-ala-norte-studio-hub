...
export const EquipmentManager = () => {
  ...
  const fetchEquipment = async () => {
    ...
    while (true) {
      const { data, error } = await supabase
        .from("equipment")
        .select(
          "id, name, name_en, image_url, images, brand, model, description, price_per_day, category_id, subcategory_id, featured, status, stock_quantity, serial_number, ownership_type, functional_status, categories:category_id (*), subcategories:category_id (*)",
        )
        .order("name")
  ...
  const handleAddImage = async (imageUrl: string) => {
    ...
    const { error } = await supabase.from("equipment_images").insert({
    ...
    if (error) {
      ...
      if (isFeatured) {
        await supabase.from("equipment").update({ image_url: imageUrl }).eq("id", selectedEquipment.id);
      }
    }
  };
  ...
  const handleRemoveImage = async (imageId: string) => {
    ...
    const { error } = await supabase.from("equipment_images").delete().eq("id", imageId);
    ...
    if (error) {
      ...
      if (imageToRemove?.is_featured && selectedEquipment) {
        ...
        if (remaining.length > 0) {
          await supabase.from("equipment_images").update({ is_featured: true }).eq("id", remaining[0].id);

          // Update legacy image_url
          await supabase.from("equipment").update({ image_url: remaining[0].image_url }).eq("id", selectedEquipment.id);
        } else {
          // No images left, clear legacy image_url
          await supabase.from("equipment").update({ image_url: null }).eq("id", selectedEquipment.id);
        }
      }
  ...
  const handleSetFeatured = async (imageId: string) => {
    ...
    await supabase.from("equipment_images").update({ is_featured: false }).eq("equipment_id", selectedEquipment.id);

    // Set new featured
    await supabase.from("equipment_images").update({ is_featured: true }).eq("id", imageId);

    // Update legacy image_url
    await supabase.from("equipment").update({ image_url: imageToFeature.image_url }).eq("id", selectedEquipment.id);
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          <CardDescription>
            Selecciona un equipo para gestionar sus imágenes. Puedes agregar múltiples imágenes y marcar una como
            destacada.
          </CardDescription>
        </CardHeader>
