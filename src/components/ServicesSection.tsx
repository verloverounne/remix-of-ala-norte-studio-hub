  ...
  return <div className="flex-[0_0_100%] min-w-0 h-screen">
      ...
      <div ref={containerRef} className="lg:hidden h-[200vh] relative">
        ...
        <div className=" h-screen z-0 duotone-hover-group" onClick={handleMobileTap}>
          {hasMedia ? isVideo ? <video ref={videoRef} src={mediaUrl!} className="sticky absolute top-4 video-duotone w-full h-full " autoPlay loop muted playsInline /> : <img src={mediaUrl!} alt={service.title} className="image-duotone w-full h-9/1 object-cover" /> : <div className="w-full h-full bg-muted flex items-center justify-center">
              <span className="font-heading text-6xl text-muted-foreground/30">
                {String(index + 1).padStart(2, "0")}
              </span>
            </div>}
        ...
        ...contentStyle
          ...
          <div className="h-screen pb-8 px-4 flex items-end justify-center">
            <div className="backdrop-blur-xl p-6 w-full bg-background/60 text-foreground max-h-[80vh] overflow-y-auto mx-4 mb-16 lg-32 xl-64 pointer-events-auto">
        ...
        <div className="container bg-[#201e1d] border-0">
          <div className="mx-0 border-0 sm:py-0 bg-transparent py-0 pt-0">
            ...
            <div className="hidden my-px mx-px flex mb:justify-between object-fill gap-0 sm:flex items-center lg:justify-between">
              {services.map((service, index) => <button key={service.id} onClick={() => handleTabClick(index)} className={cn("w-screen font-heading text-xs transition-all rounded-none shadow-none px-0 py-[16px]", activeIndex === index ? "bg-primary text-primary-foreground" : "bg-background text-foreground border border-border hover:bg-muted")}>
                  {service.title}
                </button>)}
            </div>
          </div>
