  ...
  useEffect(() => {
    ...
    function createScene() {
      ...
      const sceneHTML = `
        ...
        >
          <a-sky src="${currentImage}" rotation="180 0 0" scale="-1 1 1"></a-sky>
          ${textsHTML}
...
<body>
  <div class="top-bar">
    <h1>Sala de Grabación — Tour 360°</h1>
  ...
  <a-scene vr-mode-ui="enabled: false" loading-screen="enabled: false">
    <a-sky id="sky" src="${currentImage}" rotation="180 0 0" scale="-1 1 1"></a-sky>
  ...
  <script>
    ...
    function goBack() {
      if (window.opener) { window.close(); } else { window.location.href = '/sala-grabacion'; }
    }
